...
int WINAPI _tWinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPTSTR lpszCmdLine, int nCmdShow)
{
	...
	wcl.cbWndExtra = 0; 	
	wcl.hInstance = hInst;	
	wcl.hIcon = LoadIcon(hInst, MAKEINTRESOURCE(IDI_ICON3)); 
	wcl.hCursor = LoadCursor(hInst, MAKEINTRESOURCE(IDC_CURSOR2));
	wcl.hbrBackground = (HBRUSH) GetStockObject(WHITE_BRUSH); 
	wcl.lpszMenuName = NULL;	
	wcl.lpszClassName = szClassWindow;	
	wcl.hIconSm = LoadIcon(hInst, MAKEINTRESOURCE(IDI_ICON4));
	if (!RegisterClassEx(&wcl))
		return 0;
	hWnd = CreateWindowEx(0, szClassWindow, TEXT("Window"), WS_OVERLAPPEDWINDOW,	
		...
		case WM_MOUSEMOVE:
			{
				...
				else if (x >= rect.right / 2 && x < rect.right && y < rect.bottom / 2)
				{
					SetCursor(hCursor2);
				}
